...
func CommandConnection(log *zap.Logger, sd *statsd.Client, conn net.Conn, address string, readTimeout, writeTimeout time.Duration, id uint64, server pool.ServerWrapper, kill chan interface{}, interceptor MessageInterceptor) {
	defer func() {
		if r := recover(); r != nil {
