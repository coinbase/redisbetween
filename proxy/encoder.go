...
func NewEncoder(w io.Writer) *Encoder {
	return NewEncoderBuffer(bufio.NewWriterSize(w, 8192))
}

func NewEncoderSize(w io.Writer, size int) *Encoder {
	return NewEncoderBuffer(bufio.NewWriterSize(w, size))
}
...
type stringSlicerWriter struct {
	s []interface{}
}

func (s *stringSlicerWriter) Write(b []byte) (int, error) {
	s.s = append(s.s, string(b))
	return len(b), nil
}

func EncodeToArgs(r *Message) ([]string, error) {
	args := &stringSlicerWriter{}
	//err := Encode(args, r)
	en := NewEncoderSize(args, 1)
	err := en.Encode(r, true)
	return args.s, err
}

func (e *Encoder) encodeResp(r *Message) error {
...
func (e *Encoder) encodeMultiBulk(multi []*Message) error {
